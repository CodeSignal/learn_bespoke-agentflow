...
export class WorkflowEditor {
    ...
    renderEffortSelect(node) {
        ...
        options.forEach(optValue => {
            ...
            if (node.data.reasoningEffort === optValue)
                opt.selected = true;
    ...
    initButtons() {
        ...
        if (zoomResetBtn)
            zoomResetBtn.addEventListener('click', () => this.resetViewport());
    }
    initPanelControls() {
    ...
    openConfirmModal(options = {}) {
        const { title = 'Confirm', message = 'Are you sure?', confirmLabel = 'Confirm', cancelLabel = 'Cancel' } = options;
    ...
    renderNode(node) {
        ...
        header.addEventListener('mousedown', (e) => {
            ...
            if (interactingWithCollapse || interactingWithDelete)
                return;
            e.stopPropagation();
    ...
    getNodeLabel(node) {
        ...
        if (node.type === 'start')
            return '<span class="material-icons">play_circle</span>Start';
        if (node.type === 'end')
            return '<span class="material-icons">flag</span>End';
        if (node.type === 'if')
            return '<span class="material-icons">call_split</span>If/Else';
        if (node.type === 'approval')
            return '<span class="material-symbols-outlined">thumbs_up_down</span>User Approval';
        return node.type;
    }
    getNodePreviewText(node) {
        ...
        if (node.type === 'approval')
            return node.data.prompt || 'Approval message required';
        if (node.type === 'start')
            return 'Uses Initial Prompt';
    ...
    onConnectionLineMouseDown(e, connection, connIndex) {
        ...
        if (!sourceNode)
            return;
        let startYOffset = 24;
        if (connection.sourceHandle === 'true' || connection.sourceHandle === 'approve')
            startYOffset = 51;
        if (connection.sourceHandle === 'false' || connection.sourceHandle === 'reject')
            startYOffset = 81;
        if (connection.sourceHandle === 'output' && sourceNode.type === 'agent')
            startYOffset = 24;
    ...
    renderConnections() {
        ...
        this.connections.forEach((conn, index) => {
            ...
            if (conn.sourceHandle === 'true' || conn.sourceHandle === 'approve')
                startYOffset = 51;
            if (conn.sourceHandle === 'false' || conn.sourceHandle === 'reject')
                startYOffset = 81;
            if (conn.sourceHandle === 'output' && sourceNode.type === 'agent')
                startYOffset = 24;
    ...
    mapLogEntryToRole(entry) {
        ...
        if (type.includes('llm_response'))
            return 'agent';
        if (type.includes('input_received') || type.includes('start_prompt'))
            return 'user';
        return null;
    }
